...
public class TemaController {
    ...
    public ResponseEntity<List<TemaVagalume>> getByDescricao(@PathVariable 
    String descricao){
        return ResponseEntity.ok(temaRepository
            .findAllByDescricaoContainingIgnoreCase(descricao));
    }
	@PostMapping
    public ResponseEntity<TemaVagalume> post(@Valid @RequestBody TemaVagalume tema){
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(temaRepository.save(tema));
    }
	@PutMapping
    public ResponseEntity<TemaVagalume> put(@Valid @RequestBody TemaVagalume tema){
    ...
    public void delete(@PathVariable Long id) {
        Optional<TemaVagalume> tema = temaRepository.findById(id);
